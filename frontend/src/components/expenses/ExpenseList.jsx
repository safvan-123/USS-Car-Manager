...
//           if (!totals[p.partnerId?._id]) {
//             totals[p.partnerId?._id] = {
//               name: p.partnerId?.name || "Partner",
//               amount: 0,
//             };
//           }
//           totals[p.partnerId?._id].amount += p.amount || 0;
//         });
//       });
...
//   // 🔹 Refetch when carId OR location changes
//   useEffect(() => {
//     fetchExpenses();
//   }, [carId, location]);
...
//     // 🔹 Recalc partner totals on filtered data
//     const totals = {};
...
export default function ExpenseList() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      // 🔹 Calculate partner totals & pendings
      const totals = {};
      const pendings = {};
      res.data.forEach((exp) => {
        exp.partners?.forEach((p) => {
  ...
  useEffect(() => {
    ...
    // 🔹 Recalc partner totals & pendings on filtered data
    const totals = {};
    const pendings = {};
    filtered.forEach((exp) => {
      exp.partners?.forEach((p) => {
        ...
        if (!totals[id]) {
          ...
          pendings[id] = { name: p.partnerId?.name || "Partner", amount: 0 };
        }
        totals[id].amount += p.amount || 0;
        if (!p.paid) {
          pendings[id].amount += p.amount || 0;
                          ...
                          return (
                            ...
                            >
                              <div>
                                ...
                                <div className="col-12 col-md-6 text-start mb-1 mb-md-0">
                                  <strong>
                                    {p.partnerId?.name || "Partner"}
                                  </strong>
                                </div>
